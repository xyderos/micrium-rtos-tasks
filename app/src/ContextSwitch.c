...
OS_TMR *contextSwitchTimer;

void contextSwitchTimerCallback (OS_TMR *p_tmr, void *p_arg)
{
	printf("Hello from task timer callback!\n");
}
...
//				OSTmrStart(contextSwitchTimer, &err);
...
//			OSTmrStop(contextSwitchTimer, OS_TMR_OPT_NONE, 0, &err);
			timerRemainder = OSTmrRemainGet(contextSwitchTimer, &err);
			printf("Timer remainder is: %d\n", timerRemainder);
...
int main(void)
{
	...
	INT8U err;
	contextSwitchTimer = OSTmrCreate( 1000, // dly
		0, // 100 * 10 = 100 sec 
		OS_TMR_OPT_ONE_SHOT, // opt
		contextSwitchTimerCallback, // callback 
		(void *)0, // *callback_arg
		"Context switch timer", // *pname
		&err); //  *perr
